...
function App() {
  ...
  return <div className="app-shell">
    ...
    {view === 'maps' && activeMap && <MapDetail map={maps.find((m) => m.id === activeMap)} category={category} setCategory={setCategory} side={side} setSide={setSide} items={currentItems} links={data.links.filter((l) => l.map === activeMap && l.side === side)} onBack={() => { setActiveMap(null); setCategory(null); }} onAdd={() => setModal(category)} onEdit={(item) => { setEditing(item); setModal(category); }} onDelete={(id) => remove(category, id)} onLinkEdit={(item) => { setEditing(item); setModal('links'); }} onLinkDelete={(id) => remove('links', id)} onLinks={() => openLinks(activeMap, side, 'all')} />}
