...
function App() {
  ...
  const openLinks = () => { setView('links'); setActiveMap(null); setCategory(null); };
  ...
  return <div className="app-shell">
    ...
    {modal && <EntryModal kind={modal} initial={editing} defaultMap={activeMap || (filters.map !== 'all' ? filters.map : 'mirage')} defaultSide={side} onClose={() => { setModal(null); setEditing(null); }} onSave={(item) => save(modal, item, Boolean(editing))} />}
  </div>;
}
...
function EntryModal({ kind, initial, defaultMap, defaultSide, onClose, onSave }) {
  const [form, setForm] = useState(initial || (kind === 'tactics' ? { map: defaultMap, side: defaultSide, title: '', site: 'A', summary: '', steps: [] } : kind === 'grenades' ? { map: defaultMap, side: defaultSide, name: '', types: ['smoke'], from_position: '', target: '', summary: '' } : { title: '', url: '', map: defaultMap, side: defaultSide, type: 'tactics', nade_types: [], note: '' }));
